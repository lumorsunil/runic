...
const ScriptContext = interpreter_pkg.ScriptContext;
...
const ModuleParser = parser_pkg.module_parser.ModuleParser;
const ModuleParseError = parser_pkg.module_parser.ModuleParseError;
...
pub const InterpreterState = struct {
    ...
    pub fn init(allocator: Allocator, runner: *CommandRunner) !InterpreterState {
...
pub const ModuleRegistry = struct {
    ...
    fn parseModuleFunctions(self: *ModuleRegistry, module: *Module, source: []const u8) !void {
        var pr = ModuleParser.init(self.allocator, source);
        defer pr.deinit();
    ...
    fn parseModuleValues(self: *ModuleRegistry, module: *Module, source: []const u8) !void {
        ...
        while (iterator.next()) |raw_line| {
            ...
            if (brace_depth == 0 and std.mem.startsWith(u8, trimmed, "const")) {
...
pub fn moduleParseErrorMessage(err: anyerror) []const u8 {
    return switch (err) {
...
fn recordAstSnippet(
    ...
    const expression = parser_ptr.parseExpression() catch |err| {
        return try handleAstParseIssue(parser_ptr, err, is_control_flow, script_path, line_number, stderr);
    };
...
pub fn printScriptTokens(
    ...
    var stream = lexer_pkg.Stream.init(allocator, source);
    defer stream.deinit();
    var index: usize = 0;
    while (true) {
...
fn maybePrintAstSnippet(
    stdout: *std.Io.Writer,
    snippet: *const AstSnippet,
    print_ast: bool,
) !void {
    if (!print_ast) return;
    try stdout.print("AST {s}:{d}\n", .{ snippet.script_path, snippet.line_number });
    try renderExpressionAst(stdout, snippet.root, 0);
    try stdout.writeByte('\n');
}
...
fn renderStatementAst(writer: *std.Io.Writer, stmt: *const ast.Statement, level: usize) !void {
    switch (stmt.*) {
        ...
        .binding_decl => |let_decl| {
            try writeIndent(writer, level);
            try writer.print("let_decl @ ", .{});
            try printSpanInline(writer, let_decl.span);
            try writer.writeByte('\n');
            try writeIndent(writer, level + 1);
            try writer.print("is_mutable {}", .{let_decl.is_mutable});
            try writer.writeByte('\n');
            try renderBindingPatternAst(writer, let_decl.pattern, level + 1);
            try renderExpressionAst(writer, let_decl.initializer, level + 1);
        },
        else => {
...
fn runPipelineInternal(
    ...
    try runic.utils.forwardHandleOutput(&handle, stdout, stderr);
...
pub fn trimLineEnding(line: []const u8) []const u8 {
    if (line.len == 0) return line;
    if (line[line.len - 1] == '\r') return line[0 .. line.len - 1];
    return line;
}

pub fn isCommentLine(trimmed: []const u8) bool {
...
pub fn braceBalanceDelta(line: []const u8) isize {
    var delta: isize = 0;
    var in_string = false;
    var escape = false;
    for (line) |ch| {
...
pub fn letCommandNeedsMoreLines(command: []const u8) bool {
...
pub fn controlFlowNeedsMoreLines(command: []const u8) bool {
...
pub fn errorDeclarationNeedsMoreLines(command: []const u8) bool {
...
fn commandStartsWithLetOrMut(command: []const u8) bool {
    ...
    if (std.mem.startsWith(u8, command, "const")) {
        if (command.len == 3) return true;
        const suffix = command[3];
        return std.ascii.isWhitespace(suffix);
    }
    if (std.mem.startsWith(u8, command, "var")) {
        if (command.len == 3) return true;
        const suffix = command[3];
        return std.ascii.isWhitespace(suffix);
    }
...
pub fn isFunctionDeclaration(command: []const u8) bool {
...
pub fn computeFunctionSkipDepth(command: []const u8) isize {
...
pub fn computeScriptLabel(allocator: Allocator, script_path: []const u8) ![]u8 {
...
test "parse script invocation with tracing" {
    ...
    try std.testing.expect(!config.print_ast);
    try std.testing.expect(!config.print_tokens);
}
...
test "parse script invocation with print ast flag" {
    var gpa = std.heap.GeneralPurposeAllocator(.{}){};
    defer _ = gpa.deinit();
...
test "print ast flag requires script path" {
    var gpa = std.heap.GeneralPurposeAllocator(.{}){};
    defer _ = gpa.deinit();
    ...
    try std.testing.expect(std.meta.activeTag(result) == .usage_error);
}

test "parse script invocation with print tokens flag" {
    var gpa = std.heap.GeneralPurposeAllocator(.{}){};
    defer _ = gpa.deinit();
...
test "print tokens flag requires script path" {
    var gpa = std.heap.GeneralPurposeAllocator(.{}){};
    defer _ = gpa.deinit();
