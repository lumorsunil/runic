...
5. **Convert sourced helpers into modules.** Shared functions should move into `src/<spec>.rn` with a matching `.module.json` manifest (see `docs/module_authoring.md`). Consumers can then `import helpers from "ci/helpers"` and the loader will enforce interface contracts.
