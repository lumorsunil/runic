...
6. ✅ **FIXED — pipeline aborts on a yielded error** (pipeline cluster + abort model): a yielded error aborts the pipeline and the whole expression evaluates to it, handled by a surrounding `catch`/`try`/`match`/`||`. `collect_stdin` forwards an `.err` to the stage's stdout so the error *leads* the stream (the consumer reads it first), and `resolveSubjectType` types an error-producing pipeline as `E!<final stage>` so `(echo "abc" | parseInt | doubler | inc) catch 0` type-checks. Fixes the former comparison/`if` mis-evaluation. See `future/pipeline-errors-plan.md`. (Edges: a *bare* mid-transform pipeline isn't enforcement-flagged — Option A trade-off; downstream stage bodies still run after the error leads the stream — observationally fine for pure stages; mid-*stream* errors in a multi-value pipeline aren't first.)
