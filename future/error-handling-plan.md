...
6. ✅ **MOSTLY FIXED — mid-pipeline short-circuit** (pipeline cluster): an upstream stage's error now flows *through* downstream stages to the trailing handler — arithmetic on an `.err` propagates it (`.ath`), and `try` propagates by yielding the error to stdout. So `echo "abc" | parseInt | { … yield n*2 } catch 99` → `99`. Remaining edge: an error flowing into a *comparison* in a downstream `if` is mis-evaluated (`.cmp`/`.log` don't propagate `.err`, and the result feeds `jmp`) — niche, untested. See `future/pipeline-errors-plan.md`.
