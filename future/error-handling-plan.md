...
5. 🟡 **PARTIAL — `||`/`&&` value semantics** (7c-iv): **error-producers now work** — `mayFail || "fb"`, `echo "xyz" | parseInt || 42`, `(mayFail && "next") catch …` all capture the LHS via the typed capture (`lhsCapturesError`) so the error is observed. Still open: a **command** (`ExecutableError`) in `||`/`&&` value position yields empty output (`const x = false || echo "cap"`) — the capture-path command-output value semantics, unchanged.
