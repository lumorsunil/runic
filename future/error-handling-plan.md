...
- [x] Type check (`runTry`): subject must be error-like (else diagnostic); `TryExpr.resolveType` yields the payload type.
- [x] **Coverage enforced (2026-06-22):** `validateTryPropagation` requires the enclosing function to cover every propagated variant — its return type must be an error union whose set includes them (or an inferred `!T`, which collects). A `try` in a function whose return type is *not* an error union, or whose set is missing the variant, is a compile error, as is a top-level `try` (no enclosing function to propagate to). `ExecutableError` is exempt (commands keep the exit-code model). Closed the earlier hole where a non-error-union return type silently let the propagated error escape (it surfaced only as a confusing `catch`-site error at the caller). Diagnostics: `error_try_uncovered_return`, `error_try_top_level` (plus existing `error_try_superset`).
...
**Phases 1–9 COMPLETE**, plus the pipeline-error cluster (#5/#6/#7/#8 — see `future/pipeline-errors-plan.md`) and two language-level changes:

- **`return` removed.** Output is carried solely by `yield`; a function halts when it runs out of statements (`exit` halts early). There is no `return` keyword. (`refactor: remove the return keyword`.)
- **Mandatory error handling (Option A).** A bare statement whose result is a non-`ExecutableError` error (a value, an error-returning call, or a pipeline whose final stage yields an error union) is a compile-time `UnhandledError` — it must be `catch`/`try`/`||`'d. At the top level there's nothing to propagate to, so it must be caught; commands keep the exit-code model. `try`-propagated errors are now observable by a capturing caller (`try` yields the error to stdout + halts). `||`/`&&` observe error-producer LHSs via the typed capture.

The error-handling spec is implemented end-to-end and tested: error sets, error unions (`E!T`, inferred `!T`), value construction, `catch`/`try`/`||`/`&&`/`match`/`if`, inferred error sets, `ParseError` builtins (`parseInt`/`parseFloat`), `ExecutableError` + commands/pipelines as catchable error unions, and error values that survive the in-process capture boundary (functions, pipelines, optionals). Full CI green. Remaining items are the few **Deferred / Follow-ups** still marked open above (command `||`/`&&` value output #5; `.err` cross-*process* serialization; bare mid-transform pipeline enforcement — an Option A trade-off).
...
3. 🟢 **MOSTLY DONE — Concrete inferred error sets** (6): leading-`!T` previously used a pure *open-set* model (accepts any error, never collected). **MVP done:** the type checker now collects the variants a function body can produce into a side table (`inferred_error_sets`, keyed by the inferred `error_set` placeholder node) and `matchErrorSet` substitutes that concrete set, so **`match` exhaustiveness is enforced on inferred-fn results** and callers see a real set. Collection sources: `yield <error value>` and `try <error union>`; an `InferredErrorCollector` is pushed per function in `runFnDecl` (no AST mutation → LSP-safe; rebuilt each pass). Test: `tests/diagnostics/error_inferred_match_non_exhaustive`.
   - (a) ✅ **By design (decided 2026-06-22): whole-set granularity.** `yield E.Bad` collects *all* of `E`'s variants (`E.Bad` resolves to the whole set `E`, ast.zig:966). This is the intended behavior, not a limitation — kept for simplicity and because a yielded variant means the function deals in that error *set*.
   - (b) ✅ **DONE (2026-06-22): cross-function propagation in source order.** `collectInferredFromType` now resolves the propagated/yielded error set through `resolveInferredErrorSet`, so an inferred (`!T`) function that `try`s or yields a call to *another* inferred function inherits that function's collected variants. This covers the entire **reachable** surface: Runic has no forward function references (a bare identifier used before its declaration resolves as a *command*), so the callee always finalizes its set before the caller's body is walked, and ordinary mutual recursion can't be expressed. The only residual under-collection is direct **self-recursion** (a function's own set isn't finalized during its own body walk) — niche, would need a fixpoint pass. Tests: `error_inferred_cross_function` (diagnostic), `error_inferred_cross_function_regression` (feature).
   - (c) ✅ **DONE (2026-06-22) — `ExecutableError` from a `try`'d command** is now collected into the enclosing inferred set (`runTry`'s `.execution` branch appends `executableErrorVariants` to the active collector). Made safe by flipping `isExecutableErrorSet` to a **subset** test (every variant is a command failure mode) as part of #18: a function that both `yield E.Bad`s and `try`s a command has a mixed set that is correctly non-exempt, so `E.Bad` can't escape unhandled. Test: `error_inferred_try_command`. See item 18.
   - (d) ~~the recorded set isn't surfaced to #2 yet~~ — **done**: #2's `validateTryPropagation` consumes it via `resolveInferredErrorSet`.
4. ✅ **FIXED — `errorUnion && x`** (7c-iv): **monadic guard** semantics (chosen 2026-06-20): `a && b` evaluates to `b` when `a` is ok, or short-circuits to `a`'s error otherwise. The result is itself an error union (`E!(typeof b)`), so it must be handled (`(a && b) catch …` / propagate); a bare `a && b` is an `UnhandledError`. Implemented: `BinaryExpr.resolveType` types `.logical_and` over an error-like LHS as `E!(typeof rhs)`; `compileLogicalAndValue` (non-capture) + the `.statement`/value-capture branches of `compileLogicalBinary` lower it via `is_err` → keep error / eval rhs (mirrors the `||` discard, inverted). Also fixed `CallExpr.resolveType` to resolve a zero-arg value reference (`okv` parses as `okv()`; a non-function callee now yields the value's type instead of `null`), which is what lets the type checker see `a && b` as an error union and enforce handling. Tests: `error_and_guard_regression`, `error_and_unhandled`. (A *command* rhs in `&&`/`||` value position still yields empty output — the #5 capture-path-value limitation, unchanged.)
5. ✅ **FIXED — `||`/`&&` value semantics** (7c-iv): error-producers work — `mayFail || "fb"`, `echo "xyz" | parseInt || 42`, `(mayFail && "next") catch …` capture the LHS via the typed capture (`lhsCapturesError`). Command output in `||`/`&&` value position is captured too: `echo "a" || echo "b"` → `"a"`, and the comptime-boolean-LHS case (`const x = false || echo "cap"` → `"cap"`) was fixed — the value-capture comptime `.right` branch now types the result as the right operand's type rather than `mergedResultType(left, right)` (which was null when the discarded comptime left's type differed, losing the capture). Regression: `logical_command_value_regression`.
...
6. ✅ **FIXED — pipeline errors `pipefail`-style on a yielded error** (pipeline cluster + abort model): a yielded error makes the whole pipeline evaluate to it, handled by a surrounding `catch`/`try`/`match`/`||`. `collect_stdin` forwards an `.err` to the stage's stdout so the error *leads* the stream (the consumer reads it first), and `resolveSubjectType` types an error-producing pipeline as `E!<final stage>` so `(echo "abc" | parseInt | doubler | inc) catch 0` type-checks. Fixes the former comparison/`if` mis-evaluation. **By design (bash-consistent):** stages run concurrently and downstream bodies still execute after the error leads the stream — exactly as bash `pipefail` reports failure without stopping the other stages; a literal abort is *not* bash's behavior and isn't done. See `future/pipeline-errors-plan.md`. (Genuine edges: a *bare* mid-transform pipeline isn't enforcement-flagged — Option A trade-off; a mid-*stream* error in a multi-value pipeline isn't first.)
7. ✅ **FIXED — Error-typed function stages mid-pipeline** (pipeline cluster): the `E!T → T` pipeline boundary (`short_circuit_error`) lets an error-union stage feed a downstream `T` stage (payload crosses, error short-circuits).
8. ✅ **FIXED — `parseInt`/`parseFloat` returning an error union** (pipeline cluster): they are now `ParseError!Int` / `ParseError!Float` and produce a catchable `.err` on bad input. `echo "1234" | parseInt catch 0` → `1234`, `echo "abc" | parseInt catch 0` → `0`, `… catch |e| match e { ParseError.Invalid => … }`. Builtin `ParseError` set (ast + global type + compiler registry); `E!T → T` pipeline boundary; typed pipeline-result capture; arithmetic propagates `.err` so mid-pipeline errors flow to the trailing handler. See `future/pipeline-errors-plan.md`. Test: `parse_error_union_regression`.
...
10. ✅ **FIXED (in practice) — Variant payload type validation** (1): an unknown payload type name is now caught — `const E = error { Bad: Nonexistent }` reports "type Nonexistent not declared" (the type-expr resolution validates the payload identifier).
...
12. ✅ **FIXED (in-process) — error values across the boundary** (D2): a function/pipeline that produces an error union is captured by value via a `typed` stdout pipe + `pipe_dequeue` (`tryCompileTypedValueCapture` for calls, `pipelineCaptureErrorUnionType` for pipelines), so `catch`/`match`/`try`/`||`/`&&` operate on the real error value, not its flattened text. Extended to optionals (item 17) and to `try`-**propagated** errors (a `try` on error now yields the error to stdout + halts, so a capturing caller observes it and it chains through nested propagation). Regressions: `error_function_value_capture_regression`, `parse_error_union_regression`, `optional_function_value_capture_regression`. (Remaining: **cross-process** boundaries — external programs carry only exit code + bytes, i.e. `ExecutableError`; a real wire format is separate.)
    ...
    - **Still deferred (niche):** (a) **cross-process** boundaries (external commands) — those genuinely carry only exit code + bytes (that's `ExecutableError`); a real wire format is a separate, larger effort. (b) Two unusual capture shapes don't take the typed path: a function with `pub` *declarations inside it* (the module shape — `pub_exports.len != 0` returns a struct, not a single value) returning an error union, and a **module member-access** call (`m.fn catch x`, which today resolves the subject to the module type, not the member's return type). The common cases work: a plain `pub fn …() E!String` *is* typed-captured (catch sees the error). (Observing a `try`-**propagated** error is done — Option A made `try` yield the error to stdout.)
...
16. ✅ **FIXED — using a zero-arg function call as a value** (found during item 3):
    ...
    - ✅ **FIXED (semantics) via #12:** `catch`/`match`/`try` on an error-union-returning function now preserve the yielded error value (in-process typed capture — see item 12). `mayFail catch "fb"` → `"fb"`, `match mayFail` → its variant, `try mayOk` → unwraps; match payload-capture through `catch`-of-a-function also works (commit fixing `resolveSubjectType`). Regression: `error_function_value_capture_regression`. **Item 16 is now fully fixed.**
...
18. ✅ **DONE (2026-06-22) — Error-set merge / sum type: `const Merged = A || B`.** Mirrors Zig's `error{} || error{}`: combine two (or more, left-assoc: `A || B || C`) error sets into one whose variant list is the **union by name** of the operands', so a `Merged!T` value can carry any variant of either set. The result is an ordinary `error_set`, so every downstream consumer (`Merged.Variant` construction, `match` + exhaustiveness, `catch`/`try`/`||`/`&&`, `E!T`, the typed-value capture) works unchanged — no runtime representation change.
    - **Parsing (route ii).** Extended the **type-expr** parser to fold `A || B` (left-assoc) into a new `ast.TypeExpr.type_merge { lhs, rhs }` node (`parseMaybeTypeExpr` + `applyPostfixErrorUnion` helper). This works because `const Merged = …` already parses its RHS as a *type* expr (that's how `const E = error {…}` is distinguished from a value binding), so there's no value-`||` ambiguity; value-position `||` is parsed by the expression parser and is untouched. `(A || B)!T` inline already works via the existing paren path. Value `||` (logical-or) is unaffected.
    - **Resolution & merge.** `resolveTypeExpr` resolves `type_merge` via `resolveTypeMerge`: resolve+unalias both operands; require both `error_set` (else a clear "currently merges only error sets … general sum types not implemented" diagnostic pointing at `future/sum-types-plan.md`); union variants by name. Chained `A || B || C` flattens naturally via recursion. A duplicate variant name must carry a compatible payload (`mergeVariantPayloadsCompatible` resolves both payload type-exprs then `pipeTypesEqual`) — else "conflicting payload for variant '{s}'"; identical payloads dedup. The type binding stores the resolved `error_set`, so the raw `type_merge` node never reaches downstream switches.
    - **Closed #3(c) via the subset exemption test.** Redefined `isExecutableErrorSet` from signature-presence (had `NonZeroExit`+`SpawnFailed`) to a **subset** test: exempt iff *every* variant is one of `ExecutableError`'s command failure modes (and the set is non-empty). So pure `ExecutableError` → exempt; `ExecutableError || MyError` and any **mixed inferred set** → not exempt; pure user set → not exempt. Consistent across all callers (`isUnhandledErrorType`, `statementHasUnhandledError`, `resolveSubjectType`, `validateTryPropagation`) and verified against the existing command/ExecutableError suite. With that in place, `runTry`'s `.execution` branch now collects `ExecutableError`'s variants into the enclosing inferred set (#3c) — safe because a mixed set is correctly non-exempt.
    - **Tests.** Feature `error_set_merge_regression` (user merge, builtin merge, chained, dedup, catch/match); diagnostics `error_set_merge_non_error`, `error_set_merge_conflicting_payload`, `error_set_merge_unhandled` (subset-exemption soundness), `error_inferred_try_command` (#3c). **Deferred:** merging inline `error{…}` literals (only named/builtin sets tested); docs note general `Int || String` sum types as the separate, larger `future/sum-types-plan.md`.
