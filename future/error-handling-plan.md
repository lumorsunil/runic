...
16. 🟡 **PARTIAL — using a zero-arg function call as a value** (found during item 3):
    ...
    - **Still broken (semantics, deferred → ties into #12):** `catch`/`match`/`try` on a function whose return is an **error union** don't see the yielded *error value*. A captured function result is an `execution_result` struct, so `compileCatch`/`compileTry` run it through `compileExecutionToErrorUnion` (command semantics: exit 0 ⇒ ok = stdout text). `fn mayFail() E!String { yield E.Bad }` exits 0 and writes `"E.Bad"` to stdout, so `mayFail catch "fb"` yields `"E.Bad"` (not `"fb"`) and `match mayFail` falls to `_`. The yielded error value is flattened to stdout text and lost — the same boundary problem as **#12** (`.err` serialization across process/pipe boundaries). Needs the value model to preserve a function-produced error union across the capture (structured error channel, or exit-code+payload), not flatten it to bytes. Direct error *values*/*bindings* (`const e: E!String = E.Bad`) still work.
