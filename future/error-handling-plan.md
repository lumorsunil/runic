...
1. **Partially fixed — `catch |err|` handler on a binding subject** (7c): `compileCatchHandler` now pop-balances the capture-binding slot (same fix as #15), so `v catch |err| <handler>` runs (it used to be skipped). **Still open (re-diagnosed):** a capturing catch handler that produces output via a stdio-capture fork (e.g. `echo "${err}"`) and is the **last statement** races on program exit — its forked output isn't flushed/waited before exit. *It is NOT a closure-capture bug* (`err` is captured correctly): identical IR, works under `--verbose` (slower) and works reliably whenever a trailing statement (or any consumption) forces synchronization — which is why every committed test passes. The fix is in the fork/exit synchronization (ensure a capturing handler's fork output is drained before the enclosing statement completes). Deep concurrency work. Test `error_catch_capture_binding_regression` (with a trailing statement) is reliable.
